...
/obj/fire/Process()
	...
	if(firelevel > 6)
		icon_state = "3"
		set_light(7, 1, light_color)
	else if(firelevel > 2.5)
		icon_state = "2"
		set_light(5, 0.7, light_color)
	else
		icon_state = "1"
		set_light(3, 0.5, light_color)
...
/datum/gas_mixture/proc/can_support_fire()
	for(var/g in gas)
		if((gas_data.flags[g] & XGM_GAS_OXIDIZER) && gas[g] > 0.01)
			return TRUE
	return FALSE
