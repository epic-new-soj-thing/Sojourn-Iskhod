...
/datum/event/economic_event/start()
	...
	if(affected_dest.viable_random_events.len)
		...
		switch(event_type)
			if(RIOTS)
				dearer_goods = list(ECO_SECURITY)
			...
			if(PIRATES)
				dearer_goods = list(ECO_SECURITY, MINERALS)
			if(CORPORATE_ATTACK)
				dearer_goods = list(ECO_SECURITY, MAINTENANCE)
			if(ALIEN_RAIDERS)
			...
			if(CULT_CELL_REVEALED)
				dearer_goods = list(ECO_SECURITY, BIOMEDICAL, MAINTENANCE)
			if(SECURITY_BREACH)
				dearer_goods = list(ECO_SECURITY)
...
/datum/event/economic_event/announce()
	...
	if(!body)
		switch(event_type)
			...
			if(FESTIVAL)
				body = "A [pick("festival","week long celebration","day of revelry","planet-wide holiday")] has been declared on [affected_dest.name] by [pick("Facility Director","Commissioner","General","Commandant","Administrator")] [random_name(pick(MALE,FEMALE))] to celebrate [pick("the birth of their [pick("son","daughter")]","coming of age of their [pick("son","daughter")]","the pacification of rogue military cell","the apprehension of a violent criminal who had been terrorising the planet")]. Massive stocks of food and meat have been bought driving up prices across the planet."
