...
/datum/nano_module/digitalwarrant/nano_ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = NANOUI_FOCUS, var/datum/nano_topic_state/state = GLOB.default_state)
	...
	if(activewarrant)
		...
		var/datum/computer_file/program/P = host
		data["have_printer"] = (P && P.computer && P.computer.printer)
	else
...
/datum/nano_module/digitalwarrant/Topic(href, href_list)
	...
	if(href_list["savewarrant"])
		...
		print_warrant(user, activewarrant)
		activewarrant = null
...
/datum/nano_module/digitalwarrant/proc/print_warrant(mob/user, datum/computer_file/data/warrant/W)
	if(!user || !W)
		return
	var/datum/computer_file/program/P = host
	if(!P || !P.computer || !P.computer.printer)
		to_chat(user, "<span class='warning'>No printer connected to the console.</span>")
		return
	var/title = "[capitalize(W.fields["arrestsearch"])] Warrant - [W.fields["namewarrant"]]"
	var/raw = build_warrant_paper_text(W)
	if(!raw)
		return
	var/pencode = req_console_paperwork_to_pencode(raw)
	if(P.computer.printer.print_text(pencode, title))
		to_chat(user, "<span class='notice'>Warrant printed.</span>")
	else
		to_chat(user, "<span class='warning'>Print failed. Check printer paper.</span>")
