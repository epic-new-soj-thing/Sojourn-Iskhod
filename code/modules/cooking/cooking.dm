...
/*
Cooking - A comprehensive cooking rework.
...
/datum/cooking/recipe
	...
	/*
		...
		COOKING_BEGIN steps must eventually terminate in a matching COOKING_END step
	*/
...
/datum/cooking/recipe/proc/build_steps()
	...
	for (var/step in step_builder)
		if(islist(step))
			...
			switch(step_list[1])
				if(COOKING_ADD_ITEM)
					if(step_list.len < 2)
						reason="Bad argument Length for COOKING_ADD_ITEM"
					else if(!ispath(step_list[2]))
						reason="Bad argument type for COOKING_ADD_ITEM at arg 2"
					else
						create_step_add_item(step_list[2], FALSE)
				if(COOKING_ADD_ITEM_OPTIONAL)
					if(step_list.len < 2)
						reason="Bad argument Length for COOKING_ADD_ITEM_OPTIONAL"
					else if(!ispath(step_list[2]))
						reason="Bad argument type for COOKING_ADD_ITEM_OPTIONAL at arg 2"
					else
						create_step_add_item(step_list[2], TRUE)
				if(COOKING_ADD_REAGENT)
					if(step_list.len < 3)
						reason="Bad argument Length for COOKING_ADD_REAGENT"
					else if(!is_reagent_with_id_exist(step_list[2]))
						reason="Bad reagent type for COOKING_ADD_REAGENT at arg 2"
					else
						create_step_add_reagent(step_list[2], step_list[3], FALSE)
				if(COOKING_ADD_REAGENT_OPTIONAL)
					if(step_list.len < 3)
						reason="Bad argument Length for COOKING_ADD_REAGENT_OPTIONAL"
					else if(!is_reagent_with_id_exist(step_list[2]))
						reason="Bad reagent type for COOKING_ADD_REAGENT_OPTIONAL at arg 2"
					else
						create_step_add_reagent(step_list[2], step_list[3], TRUE)
				if(COOKING_USE_ITEM)
					if(step_list.len < 2)
						reason="Bad argument Length for COOKING_USE_ITEM"
					else if(!ispath(step_list[2]))
						reason="Bad argument type for COOKING_USE_ITEM at arg 2"
					else
						create_step_use_item(step_list[2], FALSE)
				if(COOKING_USE_ITEM_OPTIONAL)
					if(step_list.len < 2)
						reason="Bad argument Length for COOKING_USE_ITEM_OPTIONAL"
					else if(!ispath(step_list[2]))
						reason="Bad argument type for COOKING_USE_ITEM_OPTIONAL at arg 2"
					else
						create_step_use_item(step_list[2], TRUE)
				if(COOKING_ADD_PRODUCE)
					if(step_list.len < 2)
						reason="Bad argument Length for COOKING_ADD_PRODUCE"
					else
						create_step_add_produce(step_list[2], FALSE)
				if(COOKING_ADD_PRODUCE_OPTIONAL)
					if(step_list.len < 2)
						reason="Bad argument Length for COOKING_ADD_PRODUCE_OPTIONAL"
					else
						create_step_add_produce(step_list[2], TRUE)
				if(COOKING_USE_TOOL)
					if(step_list.len < 3)
						reason="Bad argument Length for COOKING_USE_TOOL"
					else
						create_step_use_tool(step_list[2], step_list[3], FALSE)
				if(COOKING_USE_TOOL_OPTIONAL)
					if(step_list.len < 3)
						reason="Bad argument Length for COOKING_USE_TOOL_OPTIONAL"
					else
						create_step_use_tool(step_list[2], step_list[3], TRUE)
				if(COOKING_USE_STOVE)
					if(step_list.len < 3)
						reason="Bad argument Length for COOKING_USE_STOVE"
					switch(step_list[2])
						if(J_LO)
							if(step_list[3] > COOKING_BURN_TIME_LOW)
								reason="Time too large for Low setting on COOKING_USE_STOVE; Food will automatically burn."

						if(J_MED)
							if(step_list[3] > COOKING_BURN_TIME_MEDIUM)
								reason="Time too large for Medium setting on COOKING_USE_STOVE; Food will automatically burn."

						if(J_HI)
							if(step_list[3] > COOKING_BURN_TIME_HIGH)
								reason="Time too large for High setting on COOKING_USE_STOVE; Food will automatically burn."

						else
							reason="Unrecognized temperature for COOKING_USE_STOVE"

					if(!reason)
						create_step_use_stove(step_list[2], step_list[3], FALSE)
				if(COOKING_USE_STOVE_OPTIONAL)
					if(step_list.len < 3)
						reason="Bad argument Length for COOKING_USE_STOVE_OPTIONAL"
					switch(step_list[2])
						if(J_LO)
							if(step_list[3] > COOKING_BURN_TIME_LOW)
								reason="Time too large for Low setting on COOKING_USE_STOVE_OPTIONAL; Food will automatically burn."

						if(J_MED)
							if(step_list[3] > COOKING_BURN_TIME_MEDIUM)
								reason="Time too large for Medium setting on COOKING_USE_STOVE_OPTIONAL; Food will automatically burn."

						if(J_HI)
							if(step_list[3] > COOKING_BURN_TIME_HIGH)
								reason="Time too large for High setting on COOKING_USE_STOVE_OPTIONAL; Food will automatically burn."
						else
							reason="Unrecognized temperature for COOKING_USE_STOVE_OPTIONAL"
					if(!reason)
						create_step_use_stove(step_list[2], step_list[3], TRUE)

				if(COOKING_USE_GRILL)
					if(step_list.len < 3)
						reason="Bad argument Length for COOKING_USE_GRILL"
					switch(step_list[2])
						if(J_LO)
							if(step_list[3] > COOKING_BURN_TIME_LOW)
								reason="Time too large for Low setting on COOKING_USE_GRILL; Food will automatically burn."

						if(J_MED)
							if(step_list[3] > COOKING_BURN_TIME_MEDIUM)
								reason="Time too large for Medium setting on COOKING_USE_GRILL; Food will automatically burn."

						if(J_HI)
							if(step_list[3] > COOKING_BURN_TIME_HIGH)
								reason="Time too large for High setting on COOKING_USE_GRILL; Food will automatically burn."

						else
							reason="Unrecognized temperature for COOKING_USE_GRILL"

					if(!reason)
						create_step_use_grill(step_list[2], step_list[3], FALSE)

				if(COOKING_USE_GRILL_OPTIONAL)
					if(step_list.len < 3)
						reason="Bad argument Length for COOKING_USE_GRILL_OPTIONAL"
					switch(step_list[2])
						if(J_LO)
							if(step_list[3] > COOKING_BURN_TIME_LOW)
								reason="Time too large for Low setting on COOKING_USE_GRILL_OPTIONAL; Food will automatically burn."

						if(J_MED)
							if(step_list[3] > COOKING_BURN_TIME_MEDIUM)
								reason="Time too large for Medium setting on COOKING_USE_GRILL_OPTIONAL; Food will automatically burn."

						if(J_HI)
							if(step_list[3] > COOKING_BURN_TIME_HIGH)
								reason="Time too large for High setting on COOKING_USE_GRILL_OPTIONAL; Food will automatically burn."
						else
							reason="Unrecognized temperature for COOKING_USE_GRILL_OPTIONAL"
					if(!reason)
						create_step_use_grill(step_list[2], step_list[3], TRUE)
				if(COOKING_USE_OVEN)
					if(step_list.len < 3)
						reason="Bad argument Length for COOKING_USE_OVEN"
					switch(step_list[2])
						if(J_LO)
							if(step_list[3] > COOKING_BURN_TIME_LOW)
								reason="Time too large for Low setting on COOKING_USE_OVEN; Food will automatically burn."

						if(J_MED)
							if(step_list[3] > COOKING_BURN_TIME_MEDIUM)
								reason="Time too large for Medium setting on COOKING_USE_OVEN; Food will automatically burn."

						if(J_HI)
							if(step_list[3] > COOKING_BURN_TIME_HIGH)
								reason="Time too large for High setting on COOKING_USE_OVEN; Food will automatically burn."

						else
							reason="Unrecognized temperature for COOKING_USE_OVEN"

					if(!reason)
						create_step_use_oven(step_list[2], step_list[3], FALSE)
				if(COOKING_USE_OVEN_OPTIONAL)
					if(step_list.len < 3)
						reason="Bad argument Length for COOKING_USE_OVEN_OPTIONAL"
					switch(step_list[2])
						if(J_LO)
							if(step_list[3] > COOKING_BURN_TIME_LOW)
								reason="Time too large for Low setting on COOKING_USE_OVEN_OPTIONAL; Food will automatically burn."

						if(J_MED)
							if(step_list[3] > COOKING_BURN_TIME_MEDIUM)
								reason="Time too large for Medium setting on COOKING_USE_OVEN_OPTIONAL; Food will automatically burn."

						if(J_HI)
							if(step_list[3] > COOKING_BURN_TIME_HIGH)
								reason="Time too large for High setting on COOKING_USE_OVEN_OPTIONAL; Food will automatically burn."
						else
							reason="Unrecognized temperature for COOKING_USE_OVEN_OPTIONAL"
					if(!reason)
						create_step_use_oven(step_list[2], step_list[3], TRUE)

				if(COOKING_USE_DEEPFRIER)
					if(step_list.len < 3)
						reason="Bad argument Length for COOKING_USE_DEEPFRIER"
					switch(step_list[2])
						if(J_LO)
							if(step_list[3] > COOKING_BURN_TIME_LOW)
								reason="Time too large for Low setting on COOKING_USE_DEEPFRIER; Food will automatically burn."
						if(J_MED)
							if(step_list[3] > COOKING_BURN_TIME_MEDIUM)
								reason="Time too large for Medium setting on COOKING_USE_DEEPFRIER; Food will automatically burn."
						if(J_HI)
							if(step_list[3] > COOKING_BURN_TIME_HIGH)
								reason="Time too large for High setting on COOKING_USE_DEEPFRIER; Food will automatically burn."
						else
							reason="Unrecognized temperature for COOKING_USE_DEEPFRIER"
					if(!reason)
						create_step_use_deepfrier(step_list[2], step_list[3], FALSE)
				if(COOKING_USE_DEEPFRIER_OPTIONAL)
					if(step_list.len < 3)
						reason="Bad argument Length for COOKING_USE_DEEPFRIER_OPTIONAL"
					switch(step_list[2])
						if(J_LO)
							if(step_list[3] > COOKING_BURN_TIME_LOW)
								reason="Time too large for Low setting on COOKING_USE_DEEPFRIER_OPTIONAL; Food will automatically burn."
						if(J_MED)
							if(step_list[3] > COOKING_BURN_TIME_MEDIUM)
								reason="Time too large for Medium setting on COOKING_USE_DEEPFRIER_OPTIONAL; Food will automatically burn."
						if(J_HI)
							if(step_list[3] > COOKING_BURN_TIME_HIGH)
								reason="Time too large for High setting on COOKING_USE_DEEPFRIER_OPTIONAL; Food will automatically burn."
						else
							reason="Unrecognized temperature for COOKING_USE_DEEPFRIER_OPTIONAL"
					if(!reason)
						create_step_use_deepfrier(step_list[2], step_list[3], TRUE)
			...
			switch(step)
				if(COOKING_BEGIN_EXCLUSIVE_OPTIONS)
					begin_exclusive_options()
				if(COOKING_END_EXCLUSIVE_OPTIONS)
					end_exclusive_options()
				if(COOKING_BEGIN_OPTION_CHAIN)
					begin_option_chain()
				if(COOKING_END_OPTION_CHAIN)
					end_option_chain()
	...
	if(last_created_step.flags & COOKING_IS_OPTIONAL)
...
/datum/cooking/recipe/proc/propagate_step_description()
	...
	if(last_created_step.flags & COOKING_IS_OPTIONAL)
		qualifier = ">> (Optional) "
	if(last_created_step.flags & COOKING_IS_OPTION_CHAIN)
		qualifier = ">> (Option Chain) "
	if(last_created_step.flags & COOKING_IS_EXCLUSIVE)
...
/datum/cooking/recipe/proc/create_step_use_oven(var/heat, var/time, var/optional)
	...
	return src.add_step(step, optional)
//-----------------------------------------------------------------------------------
//Use Deep fryer step shortcut commands
/datum/cooking/recipe/proc/create_step_use_deepfrier(var/heat, var/time, var/optional)
	var/datum/cooking/recipe_step/use_deepfrier/step = new (heat, time, src)
	return src.add_step(step, optional)
//-----------------------------------------------------------------------------------
...
/datum/cooking/recipe/proc/set_step_max_quality(var/quality)
	last_created_step.flags |= COOKING_BASE_QUALITY_ENABLED
	last_created_step.max_quality_award = quality

/datum/cooking/recipe/proc/set_step_base_quality(var/quality)
	last_created_step.flags |= COOKING_MAX_QUALITY_ENABLED
...
/datum/cooking/recipe/proc/set_exact_type_required(var/boolean)
	if((last_created_step.class == COOKING_ADD_ITEM) || (last_created_step.class == COOKING_USE_ITEM))
...
/datum/cooking/recipe/proc/set_reagent_skip(var/boolean)
	if((last_created_step.class == COOKING_ADD_ITEM) || (last_created_step.class == COOKING_ADD_PRODUCE))
...
/datum/cooking/recipe/proc/set_inherited_quality_modifier(var/modifier)
	if(last_created_step.class == COOKING_ADD_ITEM || last_created_step.class == COOKING_USE_TOOL || last_created_step.class == COOKING_ADD_PRODUCE)
...
/datum/cooking/recipe/proc/set_remain_percent_modifier(var/modifier)
	if(last_created_step.class == COOKING_ADD_REAGENT)
		last_created_step?:remain_percent = modifier
		return TRUE
	else
		return FALSE

/datum/cooking/recipe/proc/set_price_adders(var/price_to_set)
...
/datum/cooking/recipe/proc/end_exclusive_options()
	...
	for (var/datum/cooking/recipe_step/exclusive_option in active_exclusive_option_list)
		if (!GLOB.cooking_optional_step_exclusion_dictionary["[exclusive_option.unique_id]"])
			GLOB.cooking_optional_step_exclusion_dictionary["[exclusive_option.unique_id]"] = list()
	...
	for (var/datum/cooking/recipe_step/exclusive_option in active_exclusive_option_list)
		for (var/datum/cooking/recipe_step/excluder in active_exclusive_option_list["[exclusive_option]"])
			if (exclusive_option.unique_id != excluder.unique_id)
				GLOB.cooking_optional_step_exclusion_dictionary["[exclusive_option.unique_id]"] = excluder.unique_id
...
/datum/cooking/recipe/proc/add_step(var/datum/cooking/recipe_step/step, var/optional)
	...
	if(optional)
		switch(option_chain_mode)
			...
			if(1)
				...
				option_chain_mode = 2
				step.flags |= COOKING_IS_OPTION_CHAIN
	...
	//Update flags
	if(!optional)
		last_required_step.flags &= ~COOKING_IS_LAST_STEP
		step.flags |= COOKING_IS_LAST_STEP
	else
		step.flags |= COOKING_IS_OPTIONAL
		if(exclusive_option_mode)
			step.flags |= COOKING_IS_EXCLUSIVE
		if(option_chain_mode)
			step.flags |= COOKING_IS_OPTION_CHAIN
...
/datum/cooking/recipe/proc/create_product(var/datum/cooking/recipe_pointer/pointer)
	...
	if(container)
		...
		if(GLOB.cooking_step_dictionary_ordered["[COOKING_ADD_REAGENT]"])
			for(var/id in pointer.steps_taken)
				if(!GLOB.cooking_step_dictionary_ordered["[COOKING_ADD_REAGENT]"][id])
					continue
				var/datum/cooking/recipe_step/add_reagent/active_step = GLOB.cooking_step_dictionary_ordered["[COOKING_ADD_REAGENT]"][id]
			...
			for(var/obj/item/added_item in container.contents)
				...
				for(var/id in pointer.steps_taken)
					...
					if(id in exclude_list) //Only consider a step for removal one time.
						...
						#endif
						continue
					if(GLOB.cooking_step_dictionary_ordered["[COOKING_ADD_ITEM]"] && GLOB.cooking_step_dictionary_ordered["[COOKING_ADD_ITEM]"][id])
						var/datum/cooking/recipe_step/add_item/active_step = GLOB.cooking_step_dictionary_ordered["[COOKING_ADD_ITEM]"][id]
						...
						if(active_step.reagent_skip)
							...
							exclude_list += id
							break
					else if(GLOB.cooking_step_dictionary_ordered["[COOKING_ADD_PRODUCE]"] && GLOB.cooking_step_dictionary_ordered["[COOKING_ADD_PRODUCE]"][id])
						var/datum/cooking/recipe_step/add_produce/active_step = GLOB.cooking_step_dictionary_ordered["[COOKING_ADD_PRODUCE]"][id]
						exclude_specific_reagents = active_step.exclude_reagents
						if(active_step.reagent_skip)
							#ifdef COOKING_DEBUG
							log_debug("/recipe/proc/create_product: Reagent skip detected. Ignoring reagents from [added_item].")
...
/datum/cooking/proc/get_class_string(var/code)
	switch(code)
		...
		if(COOKING_USE_OVEN)
			return "Use Oven"
		if(COOKING_USE_DEEPFRIER)
			return "Use Deep Fryer"
		if(COOKING_USE_OTHER)
