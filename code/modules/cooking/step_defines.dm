...
/datum/cooking/recipe_step
	...
	var/class = COOKING_USE_OTHER //The classificaiton of the step involved.
...
/datum/cooking/recipe_step/New(var/datum/cooking/recipe/our_recipe)
	...
	if(!GLOB.cooking_step_dictionary_ordered["[class]"])
		GLOB.cooking_step_dictionary_ordered["[class]"] = list()
	GLOB.cooking_step_dictionary_ordered["[class]"]["[unique_id]"] = src
	GLOB.cooking_step_dictionary["[unique_id]"] = src
...
/datum/cooking/recipe_step/proc/calculate_quality(var/obj/added_item, var/obj/item/reagent_containers/cooking/container/container)
	return 0
...
/datum/cooking/recipe_step/proc/in_option_chain(var/datum/cooking/recipe_step/step)
	if(!step)
		return FALSE
	if(!(flags & COOKING_IS_OPTION_CHAIN) || !(step.flags & COOKING_IS_OPTION_CHAIN))
		return FALSE

	var/datum/cooking/recipe_step/target_step = src.previous_step
	//traverse backwards on the chain.
	while(target_step && !(target_step & COOKING_IS_OPTION_CHAIN))
...
/datum/cooking/recipe_step/proc/clamp_quality(var/raw_quality)
	if((flags & COOKING_BASE_QUALITY_ENABLED) && (flags & COOKING_MAX_QUALITY_ENABLED))
		return CLAMP(raw_quality, base_quality_award, max_quality_award)
	if(flags & COOKING_BASE_QUALITY_ENABLED)
		return max(raw_quality, base_quality_award)
	if(flags & COOKING_MAX_QUALITY_ENABLED)
...
/datum/cooking/recipe_step/proc/follow_step(var/obj/added_item, var/obj/item/reagent_containers/cooking/container/container)
