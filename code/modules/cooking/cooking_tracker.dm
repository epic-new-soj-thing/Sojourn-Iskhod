...
/datum/cooking/recipe_tracker/New(var/obj/item/reagent_containers/cooking/container/container)
...
/datum/cooking/recipe_tracker/proc/apply_flat_penalty(var/penalty)
...
/datum/cooking/recipe_tracker/proc/generate_pointers()
	...
	#endif
	var/obj/item/reagent_containers/cooking/container/container = holder_ref.resolve()
	...
	if(GLOB.cooking_recipe_dictionary[container.appliancetype])
		for (var/key in GLOB.cooking_recipe_dictionary[container.appliancetype])
			#ifdef COOKING_DEBUG
...
/datum/cooking/recipe_tracker/proc/process_item_wrap(var/obj/used_object, var/mob/user)
	...
	if(response == COOKING_SUCCESS || response == COOKING_COMPLETE || response == COOKING_PARTIAL_SUCCESS)
		if(!recipe_started)
			recipe_started = TRUE
	return response

//Core function that checks if a object meets all the requirements for certain recipe actions.
/datum/cooking/recipe_tracker/proc/process_item(var/obj/used_object, var/mob/user)
	#ifdef COOKING_DEBUG
	...
	for (var/datum/cooking/recipe_pointer/pointer in active_recipe_pointers)
		...
		for (var/datum/cooking/recipe_step/step in option_list)
			...
			#endif
			if(is_valid == COOKING_CHECK_VALID)
	...
	#endif
	return COOKING_PARTIAL_SUCCESS
...
/datum/cooking/recipe_tracker/proc/attempt_complete_recursive(
	...
	for (var/datum/cooking/recipe_pointer/pointer in ourlist)
		...
		for (var/datum/cooking/recipe_step/step in option_list)
			...
			if(step.check_conditions_met(used_object, src) == COOKING_CHECK_VALID)
				has_valid_step = TRUE
			else
				continue
...
/datum/cooking/recipe_pointer/New(start_type, recipe_id, var/datum/cooking/recipe_tracker/parent)
	...
	#ifdef COOKING_DEBUG
	if(!GLOB.cooking_recipe_dictionary[start_type][recipe_id])
		log_debug("Recipe [start_type]-[recipe_id] not found by tracker!")
	#endif

	current_recipe = GLOB.cooking_recipe_dictionary[start_type][recipe_id]
...
/datum/cooking/recipe_pointer/proc/get_possible_steps()
	...
	for(var/datum/cooking/recipe_step/step in current_step.optional_step_list)
		...
		if(steps_taken["[step.unique_id]"])
			...
			if(step.flags & COOKING_IS_OPTION_CHAIN)
		...
		if(step.flags & COOKING_IS_EXCLUSIVE)
			for (var/id in GLOB.cooking_optional_step_exclusion_dictionary["[step.unique_id]"])
...
/datum/cooking/recipe_pointer/proc/has_option_by_id(var/id)
	if(!GLOB.cooking_step_dictionary["[id]"])
		return FALSE
	var/datum/cooking/recipe_step/active_step = GLOB.cooking_step_dictionary["[id]"]
	var/list/possible_steps = get_possible_steps()
	if(active_step in possible_steps)
		return TRUE
	return FALSE

/datum/cooking/recipe_pointer/proc/traverse(var/id, var/obj/used_obj)
	#ifdef COOKING_DEBUG
	...
	if(!(active_step.flags & COOKING_IS_OPTIONAL))
