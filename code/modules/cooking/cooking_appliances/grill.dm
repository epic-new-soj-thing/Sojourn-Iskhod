...
/obj/machinery/cooking/grill/New()
	..()
	bsmoke.attach(src)
...
/obj/machinery/cooking/grill/proc/emit_fire()
	bsmoke.start()

//Retrieve which quadrant of the baking pan is being used.
/obj/machinery/cooking/grill/proc/getInput(params)
	...
	if(icon_x <= ICON_SPLIT_X)
		...
		input = 2
	#ifdef COOKING_DEBUG
	log_debug("cooking/grill/proc/getInput returned burner [input]. icon-x: [click_params["icon-x"]], icon-y: [click_params["icon-y"]]")
	#endif
	return input

/obj/machinery/cooking/grill/attackby(var/obj/item/used_item, var/mob/user, params)
	...
	else if(istype(used_item, /obj/item/reagent_containers/cooking/container/grill_grate))
