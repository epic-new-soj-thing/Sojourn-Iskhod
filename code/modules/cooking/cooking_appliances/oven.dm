...
/obj/machinery/cooking/oven/proc/emit_fire()
	bsmoke.start()

/obj/machinery/cooking/oven/attackby(var/obj/item/used_item, var/mob/user, params)
...
/obj/machinery/cooking/oven/proc/getInput(params)
	...
	if(icon_x >= ICON_SPLIT_X_1 && icon_x <= ICON_SPLIT_X_2 && icon_y >= ICON_SPLIT_Y_1 && icon_y <= ICON_SPLIT_Y_2)
		...
		input = FALSE
	#ifdef COOKING_DEBUG
	log_debug("cooking/oven/proc/getInput returned area [input]. icon-x: [click_params["icon-x"]], icon-y: [click_params["icon-y"]]")
	#endif
	return input
...
/obj/machinery/cooking/oven/CtrlClick(var/mob/user, params)
...
/obj/machinery/cooking/oven/CtrlShiftClick(var/mob/user, params)
...
/obj/machinery/cooking/oven/proc/handle_open(var/mob/user)
	if(opened)
		opened = FALSE
	else
		opened = TRUE
...
/obj/machinery/cooking/oven/proc/handle_temperature(var/mob/user)
...
/obj/machinery/cooking/oven/proc/handle_timer(var/mob/user)
...
/obj/machinery/cooking/oven/proc/timer_act(var/mob/user)
