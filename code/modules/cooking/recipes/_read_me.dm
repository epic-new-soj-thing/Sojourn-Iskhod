...
list(<COOKING_STEP_CLASS><_OPTIONAL>, <REQUIRED_ARGS>, <CUSTOM_ARGS>=value)
...
<COOKING_STEP_CLASS>
	...
	Valid options are:
		COOKING_ADD_ITEM
			...
			Example: list(COOKING_ADD_ITEM, /obj/item/reagent_containers/snacks/breadslice)
		...
		COOKING_ADD_REAGENT
			...
			Example: list(COOKING_ADD_REAGENT, "salt", 5)
		...
		COOKING_ADD_PRODUCE
			...
			Example: list(COOKING_ADD_PRODUCE, "banana")
		...
		COOKING_USE_TOOL
			...
			Example: list(COOKING_USE_TOOL, QUALITY_CUTTING, 5)

		COOKING_USE_ITEM
			Uses an item on the recipe. The object is not consumed.
			<REQUIRED_ARGS>:
				type_path - the type path of the item being added.
			Example: list(COOKING_USE_ITEM, /obj/item/material/kitchen/rollingpin) #Use a rolling pin on the container
		...
		COOKING_USE_STOVE
			...
			Example: list(COOKING_USE_STOVE, J_LO, 40) #Cook on a stove set to "Low" for 40 seconds.
		...
		COOKING_USE_GRILL
			...
			<REQUIRED_ARGS>:
				temperature - the required temperature to cook the food at.
					(Temperatures are macro'd by: J_LO, J_MED, J_HI)
				time - the amount of time, in seconds, to keep the food on the stove.
			Example: list(COOKING_USE_GRILL, J_LO, 40) #Cook on a stove set to "Low" for 40 seconds.

<_OPTIONAL>
	The tag _OPTIONAL can be tacked onto any command to make it an optional step not required to finish the recipe.
	Example: list(COOKING_ADD_ITEM_OPTIONAL, /obj/item/reagent_containers/snacks/breadslice)
...
<CUSTOM_ARGS>
	...
	Example: list(COOKING_ADD_ITEM, /obj/item/reagent_containers/snacks/breadslice, desc="a custom description")
	...
	Valid options are:
		...
		exact
			COOKING_ADD_ITEM or COOKING_USE_ITEM ONLY:
		...
		qmod
			COOKING_ADD_ITEM, COOKING_USE_TOOL ONLY:
		...
		reagent_skip
			COOKING_ADD_ITEM, COOKING_ADD_PRODUCE ONLY:
